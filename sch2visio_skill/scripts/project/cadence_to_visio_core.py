...
DEFAULT_WIRES_FILE = "wires.tsv"
DEFAULT_NETLIST_FILE = "netlist.cdl"
...
NODE_SIZE = 2.0 / 25.4
POWER_NET_NAME_RE = re.compile(r"^(?:A|D|P|V)?(?:VDD|VCC)(?:\d+)?$", re.IGNORECASE)
GROUND_NET_NAME_RE = re.compile(r"^(?:A|D|P|V)?(?:VSS|GND)(?:\d+)?$", re.IGNORECASE)
...
def infer_supply_nets_from_netlist(path: str) -> SupplyNets:
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for raw_line in f:
            ...
            top_cell_match = re.match(r"^\*\s*Top Cell Name:\s*(\S+)", line, re.IGNORECASE)
            if top_cell_match:
                top_cell_name = top_cell_match.group(1)
                continue
            ...
            ports = [normalize_net(token) for token in subckt_match.group(2).split() if token.strip()]
            if not ports:
                continue

            if top_cell_name and normalize_net(cell_name) == normalize_net(top_cell_name):
    ...
    power = {port for port in fallback_ports if POWER_NET_NAME_RE.match(port)}
    ground = {port for port in fallback_ports if GROUND_NET_NAME_RE.match(port)}
    return SupplyNets(power=power, ground=ground)


def _wire_segments_from_rows(rows: Iterable[Sequence[object]], path: str) -> List[WireSegment]:
    iterator = iter(rows)
    ...
    for row_no, row in enumerate(iterator, start=2):
        ...
        try:
            ...
            raise ValueError(f"Invalid wire row {row_no}: {row}") from exc
...
def read_wires_xlsx(path: str) -> List[WireSegment]:
    ...
    return _wire_segments_from_rows(ws.iter_rows(values_only=True), path)


def read_wires_tsv(path: str) -> List[WireSegment]:
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        reader = csv.reader(f, delimiter="\t")
        return _wire_segments_from_rows(reader, path)


def read_wires(path: str) -> List[WireSegment]:
    suffix = os.path.splitext(path)[1].lower()
    if suffix == ".tsv":
        return read_wires_tsv(path)
    if suffix == ".xlsx":
        return read_wires_xlsx(path)
    raise ValueError(f"Unsupported wire file format for {path}. Expected .tsv or .xlsx")
...
def write_wires_tsv(path: str, wires: Sequence[WireSegment]) -> None:
    with open(path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(["group_id", "seg_id", "net", "obj_type", "layer", "purpose", "x1", "y1", "x2", "y2"])
        for wire in wires:
            writer.writerow([
                wire.group_id,
                wire.seg_id,
...
def mos_source_junction_segments_to_restore(
    ...
    supply_net_keys = supply_nets.power | supply_nets.ground
    if not supply_net_keys:
        return segments_to_restore

    power_junctions = [
        vertex_key
        for vertex_key in endpoint_to_segments
        if vertex_key[0] in supply_net_keys
    ...
    for device in devices:
        ...
        if net_key not in supply_net_keys:
            continue
        ...
        if not same_x_junctions:
            continue
        if net_key in supply_nets.power:
        ...
        elif net_key in supply_nets.ground:
            directional_junctions = [
                junction_point
                for junction_point in same_x_junctions
                if junction_point[1] <= source_point_key[1] + PIN_ADAPTER_EPS
...
def filter_mos_body_wire_branches(
    ...
    placement_offsets: Sequence[PlacementOffsetRule] = (),
    supply_nets: Optional[SupplyNets] = None,
    ...
    segments_to_skip.difference_update(
        mos_source_junction_segments_to_restore(
            ...
            instances,
            placement_offsets,
            supply_nets or SupplyNets(power=set(), ground=set()),
        )
    )
...
def validate_inputs(
    ...
    if not wires:
        messages.append("ERROR: no wire segments parsed from wire file")
    ...
    if missing_wire_nets:
        messages.append("WARNING: nets in netlist but not in wire file: " + ", ".join(missing_wire_nets))
    if extra_wire_nets:
        messages.append("WARNING: nets in wire file but not in parsed netlist pins: " + ", ".join(extra_wire_nets))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wires", default=DEFAULT_WIRES_FILE, help="Path to wires.tsv or wires.xlsx")
    ...
    parser.add_argument(
        ...
        default="wires_adjusted.tsv",
        help="Output TSV containing wire coordinates after endpoint snapping",
    )
    parser.add_argument(
        "--adjustment-report",
...
def main() -> None:
    ...
    supply_nets = infer_supply_nets_from_netlist(args.netlist)
    wires = read_wires(args.wires)
    ...
    if not args.draw_mos_b_wires:
        if supply_nets.power or supply_nets.ground:
            print(
                "Detected supply nets from .SUBCKT: "
                f"power={', '.join(sorted(supply_nets.power)) if supply_nets.power else '(none)'}; "
                f"ground={', '.join(sorted(supply_nets.ground)) if supply_nets.ground else '(none)'}"
            )
        print(
